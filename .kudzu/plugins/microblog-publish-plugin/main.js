...
// src/networking/ErrorFactory.ts
var ErrorFactory = class {
  static async makeErrorFromResponse(response) {
    let errorDetails = "";
...
// src/networking/NewPageRequest.ts
function makeNewPageRequest(title, content, blogID, navigation) {
  return {
    "type": "h-entry",
...
// src/networking/NewPostRequest.ts
function makeNewPostRequest(blogID, title, content, categories, published, postStatus) {
  return {
    "type": ["h-entry"],
    ...blogID.length > 0 && blogID !== "default" && { "mp-destination": blogID },
    "properties": {
...
function makeUpdatePostRequest(url, blogID, title, content, categories) {
  return {
    "action": "update",
    "url": url,
    ...
    "replace": {
      "content": [content],
      ...categories !== void 0 && { "category": categories },
      "name": [title]
    }
  };
}
...
var NetworkRequestFactory = class {
  makePublishPostRequest(title, content, tags, visibility, blogID, scheduledDate) {
    const body = JSON.stringify(makeNewPostRequest(blogID, title, content, tags, scheduledDate, visibility));
    return {
      path: "/micropub",
      method: "POST",
      body
  ...
  makePublishPageRequest(title, content, blogID, navigation) {
    const body = JSON.stringify(makeNewPageRequest(title, content, blogID, navigation));
    return {
      path: "/micropub",
      method: "POST",
      body
  ...
  makeUpdateRequest(url, blogID, title, content, tags) {
    const body = JSON.stringify(makeUpdatePostRequest(url, blogID, title, content, tags));
    return {
      path: "/micropub",
      method: "POST",
      body
...
// src/views/MicropostViewModel.ts
var MicropostViewModel = class {
  ...
  publishButtonStyle() {
    ...
    if (isContentInvalid) {
      (_a = this.delegate) == null ? void 0 : _a.publishUpdateSubmitButton(0 /* Disabled */);
      return;
    }
    if (this.isSubmitting) {
      (_b = this.delegate) == null ? void 0 : _b.publishUpdateSubmitButton(1 /* Publishing */);
      return;
    }
    (_c = this.delegate) == null ? void 0 : _c.publishUpdateSubmitButton(2 /* Enabled */);
  }
...
// src/views/MicropostView.ts
var import_obsidian = require("obsidian");
var MicropostView = class extends import_obsidian.Modal {
  ...
  onOpen() {
    ...
    new import_obsidian.TextAreaComponent(contentEl).setValue(this.viewModel.content).onChange((value) => {
    ...
    if (this.viewModel.hasMultipleBlogs) {
      new import_obsidian.Setting(contentEl).setName("Blog").setDesc("Override the default blog settings for this post.").addDropdown((dropDown) => dropDown.addOptions(this.viewModel.blogs).setValue(this.viewModel.selectedBlogID).onChange((value) => {
        this.viewModel.selectedBlogID = value;
      }));
    }
    new import_obsidian.Setting(contentEl).setName("Visibility").setDesc("Override the default post visibility setting for this specific post.").addDropdown((dropDown) => dropDown.addOption("draft", "Draft").addOption("published", "Public").setValue(this.viewModel.visibility).onChange((value) => {
      this.viewModel.visibility = value;
    }));
    new import_obsidian.Setting(contentEl).addButton((button) => button.setButtonText("Post").setDisabled(true).removeCta().onClick(async (_) => {
  ...
  publishUpdateSubmitButton(style) {
...
var PublishPostViewModel = class {
  ...
  async publishNote() {
    ...
    try {
      ...
      const response = this.networkRequestFactory.makePublishPostRequest(this.title, this.content, tags, this.visibility, this.selectedBlogID, this.formattedScheduledDate());
      const result = await this.networkClient.run(response);
      this.frontmatterService.save(this.title, "title");
      this.frontmatterService.save(result.url, "url");
...
var UpdatePageViewModel = class {
  constructor(url, title, content, blogs, selectedBlogID, networkClient, frontmatterService, networkRequestFactory) {
    this.url = url;
    this.titleWrappedValue = title;
    this.content = content;
    this.blogs = blogs;
...
var UpdatePostViewModel = class {
  constructor(url, title, content, tags, blogs, selectedBlogID, networkClient, frontmatterService, networkRequestFactory, viewModelFactory) {
    this.url = url;
    this.titleWrappedValue = title;
    this.content = content;
    this.tagsWrappedValue = tags;
  ...
  async updateNote() {
    ...
    try {
      const tags = this.tags.validValues();
      const request = this.networkRequestFactory.makeUpdateRequest(this.url, this.selectedBlogID, this.title, this.content, tags);
      await this.networkClient.run(request);
      this.frontmatterService.save(this.title, "title");
      this.frontmatterService.save(tags, "tags");
  ...
  tagSuggestionDidSelectTag(category) {
    ...
    const formattedTags = tags.filter((tag, index) => index === tags.indexOf(tag)).join();
    this.tags = formattedTags;
    (_a = this.delegate) == null ? void 0 : _a.updateDidSelectTag();
  }
};
...
function isMarkdownView(value) {
  return typeof value === "object" && value instanceof import_obsidian2.MarkdownView;
}
...
String.prototype.validValues = function() {
  return this.split(",").filter((value) => value.length > 0).map((tag) => tag.trim());
};
String.prototype.removeObsidianLinks = function() {
  const linkRegex = /\[\[([^\[\]]+)\]\]/g;
  const codeBlockRegex = /```[\s\S]*?```/g;
  const inlineCodeRegex = /`[^`]+`/g;
  const extractBounds = (regex) => {
    const bounds = [];
    let match;
  ...
  const isCode = (index) => codeBlocksBounds.some((block) => index >= block.start && index <= block.end) || inlineCodesBounds.some((code) => index >= code.start && index <= code.end);
  const cleanMarkdown = (match, content, index) => isCode(index) ? match : content.includes("|") ? content.split("|")[1] : content;
...
var FrontmatterService = class {
  ...
  retrieveString(key) {
    ...
    const entry = (0, import_obsidian3.parseFrontMatterEntry)(frontmatter, key);
    return frontmatter && key in frontmatter ? entry === null || entry === "" ? "" : typeof entry === "string" ? entry : null : null;
  }
  retrieveStrings(key) {
    const frontmatter = this.parseFrontmatterFromFile();
    return (0, import_obsidian3.parseFrontMatterStringArray)(frontmatter, key);
  }
...
var TagSynchronizationService = class {
  ...
  async fetchTags() {
    ...
    if (this.settings.appToken.length == 0)
      return;
    try {
      const blogIDs = Object.keys(this.settings.blogs).filter((value) => value != "default");
      let categoriesCount = 0;
      for (const blogID of blogIDs) {
        const response = await this.networkClient.run(this.networkRequestFactory.makeCategoriesRequest(blogID));
        categoriesCount += response.categories.length;
        this.settings.synchronizedCategories[blogID] = response.categories;
        this.plugin.saveSettings();
      }
      (_a = this.delegate) == null ? void 0 : _a.tagSynchronizationDidSucceed(categoriesCount, blogIDs.length);
...
var MarkdownPage = class {
  ...
  get content() {
    return this.markdownView.editor.getValue().removeFrontmatter().removeObsidianLinks();
  }
  get url() {
...
var MarkdownPost = class {
  ...
  get title() {
    ...
    const filename = (_a = this.markdownView.file) == null ? void 0 : _a.basename;
    const frontmatterTitle = this.frontmatterService.retrieveString("title");
    return frontmatterTitle !== null ? frontmatterTitle : filename || "";
  }
  get content() {
    return this.markdownView.editor.getValue().removeFrontmatter().removeObsidianLinks();
  }
...
var MicroPluginSettingsViewModel = class {
  ...
  set synchronizedCategories(value) {
  ...
  get synchronizeCategoriesOnOpen() {
    return this.settings.synchronizeCategoriesOnOpen;
  }
  set synchronizeCategoriesOnOpen(value) {
...
var ViewModelFactory = class {
  ...
  makeSubmitPostViewModel(markdownView) {
    ...
    if (post.url && post.url.length > 0) {
      return this.makeUpdatePostViewModel(post.url, post.title, post.content, post.tags || "", frontmatterService);
    } else {
  ...
  makeTagSuggestionViewModel(blogID, excluding, delegate) {
    const suggestions = this.synchronizedCategories(blogID).filter((element) => !excluding.includes(element)).sort();
    const viewModel = new TagSuggestionViewModel(suggestions);
    viewModel.delegate = delegate;
    return viewModel;
  }
  makeMicropostViewModel() {
    return new MicropostViewModel(this.container.settings.postVisibility, this.container.settings.blogs, this.container.settings.selectedBlogID, this.container.networkClient, this.container.networkRequestFactory);
  }
  ...
  makeEmptyPageErrorViewModel() {
    return new ErrorViewModel("Oops", "Micro.blog does not support blank pages. Please write something before trying again.");
  }
  ...
  makeUpdatePostViewModel(url, title, content, tags, frontmatterService) {
    return new UpdatePostViewModel(url, title, content, tags, this.container.settings.blogs, this.container.settings.selectedBlogID, this.container.networkClient, frontmatterService, this.container.networkRequestFactory, this);
  }
  ...
  makeUpdatePageViewModel(url, title, content, frontmatterService) {
    return new UpdatePageViewModel(url, title, content, this.container.settings.blogs, this.container.settings.selectedBlogID, this.container.networkClient, frontmatterService, this.container.networkRequestFactory);
  }
...
var MicroPluginSettingsView = class extends import_obsidian5.PluginSettingTab {
  ...
  makeSettingsView() {
    ...
    new import_obsidian5.Setting(containerEl).setName("Categories synchronization").setDesc("Toggle on to automatically synchronize categories when Obsidian opens.").addToggle((toggle) => toggle.setValue(this.viewModel.synchronizeCategoriesOnOpen).onChange((value) => {
...
var UpdatePostView = class extends import_obsidian10.Modal {
  ...
  onOpen() {
    ...
    if (this.viewModel.hasMultipleBlogs) {
      new import_obsidian10.Setting(contentEl).setName("Blog").setDesc("Confirm the blog for this post.").addDropdown((dropDown) => dropDown.addOptions(this.viewModel.blogs).setValue(this.viewModel.selectedBlogID).onChange((value) => {
        this.viewModel.selectedBlogID = value;
      }));
    }
    new import_obsidian10.Setting(contentEl).setName("Categories").setDesc("Confirm the categories assigned to this post.").addText((text) => text.setPlaceholder("category1, category2, category3").setValue(this.viewModel.tags).onChange((value) => {
      this.viewModel.tags = value;
    })).addExtraButton((button) => button.setIcon("plus").setTooltip("Add categories").onClick(() => {
      new TagSuggestionView(this.viewModel.suggestionsViewModel(), this.app).open();
    }));
...
var MicroPlugin = class extends import_obsidian11.Plugin {
  ...
  tagSynchronizationDidSucceed(count, blogsCount) {
    new import_obsidian11.Notice("Categories synchronized. Found " + count + " categories in " + blogsCount + " blog(s).");
  }
